...
void print_compacted_memory_ranges(pml4e_t *pml4) 
{
    ...
    for (int i = 0; i < 512; i++) {
        ...
        if (!pml4e) continue;
        pdpe_t *pdpe = (pdpe_t *)(pml4e & 0x7FFFFFFFFFFFF000);
        for (int j = 0; j < 512; j++) {
            pdpe_t pdpe_entry = pdpe[j];
            if (!pdpe_entry) continue;

            pde_t *pde = (pde_t *)(pdpe_entry & 0x7FFFFFFFFFFFF000);
            for (int k = 0; k < 512; k++) {
                ...
                if (pde_entry & (1ull << 7)) {
                    ...
                    if (!range_started) {
                        ...
                        // Print the previous range with flags and raw flag
                    ...
                    pte_t *pte = (pte_t *)(pde_entry & 0x7FFFFFFFFFFFF000);
                    for (int l = 0; l < 512; l++) {
    ...
    if (range_started) {
        // Print the last range with flags and raw flag
        printf("%#18lx - %#18lx Flags: R:%d W:%d X:%d Cache:%d [RAW: %#lx]\n", 
               from, to, 
               (flags & 0x1),      // Read
               (flags & 0x2) >> 1, // Write
